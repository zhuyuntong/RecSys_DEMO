...
def main(args):
    ...
    # merged_pairs_df = pd.read_csv('yelp_combined.csv') # FIXME
    print(f"Elapsed time: {time.time() - start_time:.2f} seconds\n")
    ...
    # BUG
    # val_df = val_spark_df.toPandas()  # 转换为Pandas DataFrame
    # BUG
    ...
    '''
    # DEBUG
    # 遍历所有clusters并检查每个cluster的数据
    # for size, cluster_list in clusters.items():
    ...
    #             print(f"Cluster {cluster} (size {size}) is empty.")
    #         else:
    #             print(f"Cluster {cluster} (size {size}) contains {len(train_cluster_data)} rows.")
    #             print(train_cluster_data.head())
    # DEBUG
    # exit(1)


    def prepare_data_for_prediction(df, columns_to_drop, required_columns=None):
        if required_columns is not None:  # 确保required_columns非空才进行迭代
            missing_columns = [col for col in required_columns if col not in df.columns]
            if missing_columns:
                raise ValueError(f"Missing required columns: {', '.join(missing_columns)}")

        # remove cols fro training
        existing_columns = [col for col in columns_to_drop if col in df.columns]
        return df.drop(existing_columns, axis=1)

    columns_to_drop = ['stars', 'user_id', 'business_id', 'Cluster']
    required_columns = ['user_id', 'business_id', 'Cluster']
    ...
    #################################################################################################
    # # Merge all results of val_data
    # all_predictions = pd.DataFrame()
    # for cluster in large_clusters + moderate_clusters + small_clusters:
    #     val_cluster_data = val_df[val_df['Cluster'] == cluster]
    #     # cluster check, if empty, pass
    #     if cluster in models:
    #         if val_cluster_data.empty:
    #             print(f"Skipping cluster {cluster} due to empty labels in testing set.")
    #             continue

    #         cluster_X_val = prepare_data_for_prediction(val_cluster_data, ['stars', 'user_id', 'business_id', 'Cluster'])
    #         print(cluster_X_val)
    #         print(cluster_X_val.columns)
    #         print(val_cluster_data.columns)

    #         predictions = models[cluster].predict(cluster_X_val)
    #         result_df = pd.DataFrame(
    #             {
    ...
    #         )
    #     else:
    #         print(f"No model trained for cluster {cluster}, skipping predictions.")

    #     # add current cluster prediction to final predictions
    #     all_predictions = pd.concat([all_predictions, result_df], ignore_index=True)
    ######################################################################################################
    # final sanity check
