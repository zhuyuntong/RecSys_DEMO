...
class FeatureProcessor:
    ...
    def process_all_features(self, sc, train_df, pair_file_path='../yelp_combined.csv'):
        ...
        # user_columns_to_add = [col for col in self.user_df.columns if col not in train_df.columns and col != 'user_id']
        # business_columns_to_add = [
        #     col for col in self.business_df.columns if col not in train_df.columns and col != 'business_id'
        # ]

        # user_columns_to_add = [col for col in self.user_df.columns if col not in train_df.columns]
        # business_columns_to_add = [col for col in self.business_df.columns if col not in train_df.columns]

        # final_train_df = pd.merge(final_train_df, self.user_df[user_columns_to_add], on='user_id', how='left')
        # final_train_df = pd.merge(final_train_df, self.business_df[business_columns_to_add], on='business_id', how='left')
        ...
        # final_train_df = pd.merge(final_train_df, self.user_df[user_columns_to_add + ['user_id']], on='user_id', how='left')
        # final_train_df = pd.merge(
        #     final_train_df, self.business_df[business_columns_to_add + ['business_id']], on='business_id', how='left'
        # )

        def inverse_log_transform(normalized_score):
            """逆向转换标准化后的评分到原始亲和力分数"""
            max_log = np.log(284 + 1)  # 最高的implicit互动次数
            min_log = np.log(1 + 1)  # 最低次数
