...
# DEBUG AND FINE TUNE PURPOSE
def test(sc, data_folder_path, pair_file_path='../yelp_combined.csv', rank=150, reg_param=0.0236, alpha=1725.6):
...
def decompose_matrix_factorization_als(sc, data_folder_path, rank=51, reg_param=30, alpha=1530.0):  # FIXME:
    # def decompose_matrix_factorization_als(sc, data_folder_path, rank=150, reg_param=0.0236, alpha=1725.6):
    # Helper function to parse JSON data
    def parse_json(line):
        try:
            data = json.loads(line)
    ...
    user_business_interaction_counts.unpersist()
    ...
    user_features_rdd.unpersist()
    biz_features_rdd.unpersist()
...
def predict_aff_score_als(
    ...
    # Track pairs assigned default -1 and grant with default ratings
    ...
    valid_num_id_pairs.unpersist()
    valid_batches.unpersist()
    valid_scores_rdd.unpersist()
    mapped_scores_rdd.unpersist()
    mapped_missing_scores_rdd.unpersist()
