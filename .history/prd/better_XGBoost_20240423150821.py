...
def main(args):
    ...
    for size in clusters:
        for cluster in clusters[size]:
            ...
            if size == 'large':
                ...
                model.fit(X_train, y_train, early_stopping_rounds=5, verbose=False)
    ...
    for cluster in large_clusters + moderate_clusters + small_clusters:
        ...
        predictions = models[cluster].predict(val_cluster_data.drop(['user_id', 'business_id', 'Cluster'], axis=1))
    ...
    merged_df.to_csv(output_file, columns=['user_id', 'business_id', 'prediction'], index=False)
